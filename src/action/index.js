...
import {SIGN_IN,SIGN_OUT, CREATE_USER, LOGIN,CREATE_ITEM,FETCH_ITEM,FETCH_ITEMS,DELETE_ITEM,ADD_ITEM_CART,EMPTY_CART, DELETE_ITEM_CART,WHAT_SYSTEM,SHOW_ADD_ITEM_TO_CART}  from './types';
...
if(res.data==='getout'){
    dispatch({type:LOGIN,payload:'getout'})
}
...
export const loginUser=(formValues,signIn)=> async dispatch=>{
   ...
   export const addItemToStore=(item,count)=>dispatch=>{
    dispatch({
        type:ADD_ITEM_CART,
        payload:{item,count}
    })
   ...
   export const changeShowAddItem=(state)=>dispatch=>{
    dispatch({
        type:SHOW_ADD_ITEM_TO_CART,
        payload:state
    })
   }
